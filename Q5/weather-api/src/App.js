  ...
  const findWeather = () => {
    ...
    if (
      ...
      try {
        ...
        fetch(
          `${apiKey.link}weather?q=${city},${country}&units=metric&appid=${apiKey.key}`
        )
          .then((res) => res.json())
          .then((result) => {
            // console.log(result);
    ...
    <Fragment>
      <div id="mainContainer">
        <main>
          ...
          <div id="searchForm">
            <h3>Country name</h3>
            <input type="text" required onChange={funcSetCountry} />
            <h3>City name</h3>
            <input type="text" required onChange={funcSetCity} />
            <br />
            <br />
